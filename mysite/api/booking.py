...
async def detail_booking(room_id: int, db: Session = Depends(get_db)):
    booking_db = db.query(Booking).filter(Booking.id == room_id).first()
    if not booking_db:
        raise HTTPException(detail='Мындай маалымат жок', status_code=400)

    return booking_db

@booking_router.put(path='/booking_id}/', response_model=dict)
